...
void leds_blink(int led_number) {
...
void leds_display_winner() {
  ...
  for (uint8_t lane = 0; lane < 2; ++lane) {
    for (uint8_t i = 0; i < LANE_LEDS; ++i) {
      leds[LANE_LEDS * lane + i] = (lane == 0) ? lane_a_color : lane_b_color;
    }
  }
}
...
void loop() {
  ...
  if (in_menu) {
    if (!prev_in_menu) {
      ...
      lcd.setCursor(0, 0);
      lcd.print("Menu");
    } else if (button_up.read() == single_click) {
      lcd.setCursor(0, 1);
      lcd.print("bla");
    }
  } else if (!in_menu && prev_in_menu) {
    full_reset();
  } else {
    speed_measure_loop();
  }
